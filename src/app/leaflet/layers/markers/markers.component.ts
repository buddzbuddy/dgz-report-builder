...
import { icon, latLng, Layer, marker, tileLayer, LeafletEventHandlerFn } from 'leaflet';
@Component({
...
export class MarkersComponent {
  ...
  addMarker() {
    ...
    newMarker.bindPopup('<h3>Marker ' + this.markers.length + '</h3>');
    newMarker.on('click', this.setLatLng.bind(this));
    newMarker.on('drag', this.setLatLng.bind(this));
